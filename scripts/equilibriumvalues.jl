...
equilparms = let 
    ...
    # `durations` for plotting effect of changing mean duration of immunity set to provide 
    # finer resolution for first 2.5 years
    durations = [collect(0.01:0.01:2.5); collect(2.5:0.1:80)] 
    R0s = collect(0.0125:0.025:15)
...
let 
    ...
    for (i, β) ∈ enumerate(betas), (j, ϕ) ∈ enumerate(psis)
        I = equili(SirnsParameters(; β0=β, γ, μ, ϕ, ω))
        if 0 < I <= 1   # endemic equilibrium
            equilSs[i, j] = equils(SirnsParameters(; β0=β, γ, μ, ϕ, ω))
            equilIs[i, j] = I
            equilRs[i, j] = equilr(SirnsParameters(; β0=β, γ, μ, ϕ, ω), I) 
...
critvector = let 
    ...
    [
        [
            realmaxequileigen(SirnsParameters(; β0=β, γ, μ, ψ, ω); warntol=Inf) 
            for ψ ∈ psis, ω ∈ omegas
        ] 
        for β ∈ betas
    ]
...
stablesim = let
    ...
    ϕ = 0.5
    β = R0 * (γ + μ)
    p = SirnsParameters(; β0=β, γ, μ, ϕ, ω)
    u0 = sirns_u0(0.8, 0.03; p, equalrs=true)
...
let
    ...
    let
        u0 = sirns_u0(0.7, 0.1; p, equalrs=true)
        tspan = (-1000, 10)
        sol = run_sirns(u0, p, tspan)
    ...
    let
        u0 = sirns_u0(0.64, 0.008; p, equalrs=true)
        tspan = (0, 3.8)
        sol = run_sirns(u0, p, tspan)
...
let
    ...
    # approach the cycle from inside
    let
        u0 = sirns_u0(0.75, 0.01; p, equalrs=true)
        tspan = (0, 3.2)
        sol = run_sirns(u0, p, tspan)
...
let 
    ...
    let 
        ϕ = 0.5 
        p = SirnsParameters(; β0=β, γ, μ, ϕ, ω)
        u0 = sirns_u0(0.7, 0.01; p, equalrs = true)
        sol = run_sirns(u0, p, tspan)
